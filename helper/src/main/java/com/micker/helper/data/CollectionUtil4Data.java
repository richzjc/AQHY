 ...
 * @since 1.1.3
 */
public abstract class CollectionUtil4Data {
	 ...
	 * @see ObjectUtil4Data#toObjectArray(Object)
	 */
	public static List arrayToList(Object source) {
		return Arrays.asList(ObjectUtil4Data.toObjectArray(source));
	}
	...
	public static void mergeArrayIntoCollection(Object array, Collection collection) {
		...
		Object[] arr = ObjectUtil4Data.toObjectArray(array);
		for (Object elem : arr) {
			collection.add(elem);
		}
	}
	...
	public static boolean contains(Iterator iterator, Object element) {
		if (iterator != null) {
			while (iterator.hasNext()) {
				...
				if (ObjectUtil4Data.nullSafeEquals(candidate, element)) {
					return true;
				}
			}
		}
	...
	public static Object findValueOfType(Collection<?> collection, Class<?>[] types) {
		if (isEmpty(collection) || ObjectUtil4Data.isEmpty(types)) {
			return null;
		}
	...
	public static <E> List<E> subList(List<E> list, int start, int end) {
		...
		int ms = Math.max(0, start);
		int me = Math.min(list.size() - 1, end);
		return list.subList(ms, me);
	}
